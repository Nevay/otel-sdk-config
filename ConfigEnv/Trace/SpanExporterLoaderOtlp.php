...
final class SpanExporterLoaderOtlp implements EnvComponentLoader {
    ...
    public function load(EnvResolver $env, EnvComponentLoaderRegistry $registry, Context $context): SpanExporter {
        ...
        $format = match ($env->enum('OTEL_EXPORTER_OTLP_TRACES_PROTOCOL', ['http/protobuf', 'http/json', 'grpc']) ?? $env->enum('OTEL_EXPORTER_OTLP_PROTOCOL', ['http/protobuf', 'http/json', 'grpc']) ?? 'http/protobuf') {
            'http/protobuf' => ProtobufFormat::Protobuf,
            'http/json' => ProtobufFormat::Json,
            'grpc' => null,
        };
        $endpoint = $format
            ? $env->string('OTEL_EXPORTER_OTLP_TRACES_ENDPOINT') ?? ($env->string('OTEL_EXPORTER_OTLP_ENDPOINT') ?? 'http://localhost:4318') . '/v1/traces'
            : $env->string('OTEL_EXPORTER_OTLP_TRACES_ENDPOINT') ?? $env->string('OTEL_EXPORTER_OTLP_ENDPOINT') ?? 'http://localhost:4317';
        ...
        if (!$format && parse_url($endpoint, PHP_URL_SCHEME) === null) {
            $insecure = $env->bool('OTEL_EXPORTER_OTLP_TRACES_INSECURE') ?? $env->bool('OTEL_EXPORTER_OTLP_INSECURE') ?? false;
            $scheme = $insecure
                ? 'http'
                : 'https';
