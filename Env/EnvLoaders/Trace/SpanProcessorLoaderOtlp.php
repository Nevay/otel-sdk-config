...
final class SpanProcessorLoaderOtlp implements Loader {
    ...
    public function load(EnvResolver $env, LoaderRegistry $registry, Context $context): SpanProcessor {
        ...
        if ($clientCertificate = $env->path('OTEL_EXPORTER_OTLP_TRACES_CLIENT_CERTIFICATE') ?? $env->path('OTEL_EXPORTER_OTLP_CLIENT_CERTIFICATE')) {
            $tlsContext = $tlsContext->withCertificate(new Certificate($clientCertificate, $env->path('OTEL_EXPORTER_OTLP_TRACES_CLIENT_KEY') ?? $env->path('OTEL_EXPORTER_OTLP_CLIENT_KEY')));
        }
        if ($certificate = $env->path('OTEL_EXPORTER_OTLP_TRACES_CERTIFICATE') ?? $env->path('OTEL_EXPORTER_OTLP_CERTIFICATE')) {
